    ...
    with open("./tweet_load.csv", mode="r", encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            if line - 2 == load_data_length:
                break
            input_stream.append(int(row[1]) // 20)
...
plt.plot([i for i in range(len(input_stream[:1440]))], input_stream[:1440], label="Tweet Trace")
...
mobilenet_input_stream = list(input_stream[n:2*n])
...
vgg19_input_stream = list(input_stream[5*n:6*n])
...
plt.ylabel("Request Number", fontdict=font1)
plt.legend(prop=font1, loc=2, markerscale=1,)
