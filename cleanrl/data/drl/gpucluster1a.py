...
class GPUEnv1a(gym.Env[np.ndarray, Union[int, np.ndarray]]):
    # 重置buffer状态
    def defineBuffers(self):
        ...
        self.buffers['alexnet'] = []
        self.buffers['inception'] = []
    ...
    def __init__(self):
        ...
        def defineInputStream(self):
            ...
            self.input_streams['alexnet'] = [i for i in range(250, 270)]
            self.input_streams['inception'] = [i for i in range(150, 170)]
        ...
        def defineDiscardRequest(self):
            ...
            self.discard_request_nums['alexnet'] = [0 for _ in range(20)]
            self.discard_request_nums['inception'] = [0 for _ in range(20)]
        ...
        self.action_length = int(math.pow(GPU_NUMS * len(candidate_batch), MODEL_NUMS))
        ...
        self.model_names = ['resnet50', 'vgg19', 'densenet201', 'mobilenet', 'alexnet', 'inception']                                              
...
# ---------------- 实际初始化（游戏结束需要重置的部分） -------------------
        ...
        for m_idx in range(len(self.model_names)):
            for g_idx in range(len(self.GPUS)):
        ...
        for full_config in tmp_res:
            ...
            if len(name_set) == MODEL_NUMS:
                ...
                cnt += 1
        print(len(tmp_res), len(self.action_config_map))
    ...
    def step(self, action): 
        ...
        def checkThroughput(self):
            ...
            throughput['alexnet'] = 0
            throughput['inception'] = 0
            for GPU in self.GPUS:
                for place in GPU.places:
