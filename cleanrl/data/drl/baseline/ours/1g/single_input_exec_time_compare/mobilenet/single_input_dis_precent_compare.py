# 该文件绘制基准输入流大小从100 120 140 160，4种情况下的MobileNet模型实际执行时间对比图
...
def tensorboard_smoothing(values, smooth = 0.5):
    """不需要传入step"""
...
total_df_origin_data = pd.DataFrame({'data':[0 for _ in range(4000)], 'x':[0 for _ in range(4000)], 'color':[0 for _ in range(4000)]})
total_df_smoothed_data = pd.DataFrame({'data':[0 for _ in range(4000)], 'x':[0 for _ in range(4000)], 'color':[0 for _ in range(4000)]})
...
mark = [72 * i + 12 for i in range(0, 14)]
...
plt.ylabel('Model Execution Time', fontdict=font1)
plt.tight_layout()
plt.savefig("./{}_load_times_compare_900k.pdf".format(model_name))
plt.savefig("./{}_load_times_compare_900k.png".format(model_name))
plt.show()
print("-------------------------------")
