...
total_df_origin_data = pd.DataFrame({'data':[0 for _ in range(1248)], 'x':[0 for _ in range(1248)], 'color':[0 for _ in range(1248)]})
total_df_smoothed_data = pd.DataFrame({'data':[0 for _ in range(1248)], 'x':[0 for _ in range(1248)], 'color':[0 for _ in range(1248)]})
...
for idx, ascend in enumerate([0, 20, 40, 60, 80, 150]):    
...
palette = sns.color_palette(["#E34933","#FCA532","#FEE90C","#A3DDE6","#588CC0","#9646C8"])
...
sns.lineplot(x="x", y="data", data=total_df_origin_data, hue='color', palette=palette, legend=False, alpha=0.2)
...
plt.xticks([0, 40, 80, 120, 160, 200], ['0', '80k','160k','240k', '320k', '400k'])
...
plt.tight_layout()
plt.savefig("./{}_400k.pdf".format(model_name))
plt.savefig("./{}_400k.png".format(model_name))
plt.show()
print("-------------------------------")
