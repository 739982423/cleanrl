...
# ------------------ 这里是要修改的baseline部分 --------------------
    ax[r][c].hlines(xmin=0, xmax=208, y=min_workload_dis_percent_matrix[4][idx + 1], label="Least Loaded", linestyles=":", colors='violet',lw=2)
    ax[r][c].hlines(xmin=0, xmax=208, y=igniter_dis_percent_matrix[4][idx + 1], label="IGniter+", linestyles="--", colors='k',lw=2)
# -------------------------------------------------------------------
...
plt.savefig("./base290_{}_dis_precent_ig_le_compare_400k.pdf".format(model_name))
plt.savefig("./base290_{}_dis_precent_ig_le_compare_400k.png".format(model_name))
plt.show()
print("-------------------------------")
