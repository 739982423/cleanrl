# 该文件绘制基准输入流大小从210 230 250 270 290 390，6种情况下的DenseNet201请求丢弃率对比图
...
for idx, ascend in enumerate([0, 20, 40, 60, 80, 180]):    
    ...
    for csv in files:
        ...
        line_type = None
        if data_file == "base210":
            line_type = "Base"
        else:
            line_type = "Base+" + str(int(data_file[-3:]) - 210)
        ...
        for i in range(416):
            ...
            total_df_smoothed_data.loc[total_df_smoothed_data_cnt, 'color'] = line_type
        ...
        for i in range(416):
            ...
            total_df_origin_data.loc[total_df_origin_data_cnt, 'color'] = line_type
...
pattle = sns.color_palette("RdYlBu", 6)
# pattle = sns.color_palette('colorblind')
...
min_workload_dis_percent_matrix = [
                                    ...
                                    [180,0.005100135505692306,0.12755335714940003,0.15581273483404132,0.0]
                                  ]
...
igniter_dis_percent_matrix = [
                                ...
                                [180,0.02407544572836977,0.0675290830814326,0.0568059225563323,0.0]
                             ]

sns.lineplot(x="x", y="data", data=total_df_origin_data, hue='color', palette=pattle, legend=False, alpha=0.2)
s = sns.lineplot(x="x", y="data", data=total_df_smoothed_data, style='color', hue='color', dashes=False, linestyle="-", 
                ...
                palette=pattle,  alpha=1)
# plt.hlines(xmin=0, xmax=400, y=0.12755335714940003, label="Least Loaded", linestyles=":", colors='violet',lw=2)
# plt.hlines(xmin=0, xmax=400, y=0.0568059225563323, label="IGniter", linestyles="--", colors='k',lw=2)
...
plt.title(model_name, fontdict=font1)
plt.grid(linestyle='-.')
