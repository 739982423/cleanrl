...
def tensorboard_smoothing(values, smooth = 0.95):
    """不需要传入step"""
...
for mode in ["wlstm", "wolstm"]:
    ...
    for idx, m in enumerate(file_model_names):
        ...
        for csv in files:
            if csv[-4:] != ".csv":
                continue
            df = pd.read_csv("./{}/{}/{}/{}".format(mode, "base210", m, csv))
            ...
            if mode == 'wlstm':
                ...
                color = "#4C72B0"
            ax[r][c].plot([i for i in range(len(list(total_df_origin_data['data'])))], list(total_df_origin_data['data']), color=color, alpha=0.15)
            ax[r][c].plot([i for i in range(len(list(total_df_smoothed_data['data'])))], list(total_df_smoothed_data['data']), color=color, marker='o', 
...
for idx, m in enumerate(file_model_names):
    ...
    print(idx, m)
# ------------------ 这里是要修改的baseline部分 --------------------
    ax[r][c].hlines(xmin=0, xmax=520, y=min_workload_dis_percent_matrix[0][idx + 1], label="Least Loaded", linestyles=":", colors='violet',lw=2)
    ax[r][c].hlines(xmin=0, xmax=520, y=igniter_dis_percent_matrix[0][idx + 1], label="IGniter+", linestyles="--", colors='k',lw=2)
...
plt.savefig("./base210_{}_dis_precent_ig_le_compare_1M.pdf".format(model_name))
plt.savefig("./base210_{}_dis_precent_ig_le_compare_1M.png".format(model_name))
plt.show()
print("-------------------------------")
