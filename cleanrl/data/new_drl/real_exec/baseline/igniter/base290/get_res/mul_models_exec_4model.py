...
if __name__ == "__main__":
    ...
    parser.add_argument("-m3", "--model3", help="需要被测试的model3", type=int)
    parser.add_argument("-m4", "--model4", help="需要被测试的model4", type=int)
    parser.add_argument("-b1", "--batch1", help="传入的batch1", type=int)
    parser.add_argument("-b2", "--batch2", help="传入的batch2", type=int)
    parser.add_argument("-b3", "--batch3", help="传入的batch3", type=int)
    parser.add_argument("-b4", "--batch4", help="传入的batch4", type=int)
    parser.add_argument("-g1", "--gpu1", help="分配的GPU资源1", type=int)
    parser.add_argument("-g2", "--gpu2", help="分配的GPU资源2", type=int)
    parser.add_argument("-g3", "--gpu3", help="分配的GPU资源3", type=int)
    parser.add_argument("-g4", "--gpu4", help="分配的GPU资源4", type=int)
    ...
    # 这两个变量与同时执行的模型数量有关
    barrier = Barrier(4)        
    TotalModels = Value('i', 4) # 表示当前同时执行的模型数量
    ...
    p3 = Process(target=run, args=(args.model3, barrier, args.batch3, args.gpu3, ratio, model_exec_times, tmp_csv_path))
    p4 = Process(target=run, args=(args.model4, barrier, args.batch4, args.gpu4, ratio, model_exec_times, tmp_csv_path))
    ...
    server_id = os.popen("echo get_server_list | nvidia-cuda-mps-control").readlines()[0].strip('\n')
    act_thread1 = os.popen("echo get_active_thread_percentage {} | sudo nvidia-cuda-mps-control".format(server_id)).readlines()[0].strip('\n')
    gpu_set_cmd = "echo set_active_thread_percentage {} {} | sudo nvidia-cuda-mps-control".format(server_id, args.gpu4)
    os.system(gpu_set_cmd)
    act_thread2 = os.popen("echo get_active_thread_percentage {} | sudo nvidia-cuda-mps-control".format(server_id)).readlines()[0].strip('\n')
