...
if __name__ == "__main__":
    env = gym.make('GPUcluster-1a_mh_p_4m_2g_real', evaluation_flag = True, input_ascend = 80)
    device = torch.device('cuda')

    origin_action_list = []
    with open("base210_action_list_ig.csv", mode="r", encoding="utf-8-sig", newline="") as f:
        reader = csv.reader(f)
        for row in reader:
            origin_action_list.append(row)

    action_list = []
    for t, row in enumerate(origin_action_list):
        model1_name = row[0]
        model2_name = row[3]
        model3_name = row[6]
        model4_name = row[9]
        ...
        action_list.append(cur_exec_action)
    ...
    while(not done):
        ...
        idx += 1
        next_state, reward, done, extra_message = env.step_igniter((cur_action, 255))
        s = next_state
