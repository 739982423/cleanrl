...
for idx, model_name in enumerate(model_names):
    ...
    if model_name == "mobilenet":
        GPU0_throughput = [ratio_throughput_list[i][0] / 3 for i in range(time_duration)]
        GPU1_throughput = [ratio_throughput_list[i][1] / 3 for i in range(time_duration)]
        ...
        GPU0_throughput = [ratio_throughput_list[i][0] for i in range(time_duration)]
        GPU1_throughput = [ratio_throughput_list[i][1] for i in range(time_duration)]
    ...
    plt.plot(x, GPU1_throughput, label="GPU1")
    plt.fill_between(x=x, y1=0, y2=GPU1_throughput, alpha=0.5)
    ...
    plt.legend()
    plt.savefig("{}_{}.png".format("base210", model_name))
    plt.show()
