...
def process_exec_action(exec_action):
    ...
    for i in range(len(config)):
        ...
        if cur_model_name != "mobilenet":
            cur_model_batch = config[i][2] * 2
        else:
    ...
    for gpu_idx, tmp_gpu in enumerate(tmp_gpus):
        ...
        for idx, model_data in enumerate(tmp_gpu):
            ...
            cur_model_batch = model_data[1] * 2
...
if __name__ == "__main__":
    ...
    # 读取动作列表
    action_list = []
    with open("./../base290_action_list_ll.csv", mode="r", encoding="utf-8-sig") as f:
        reader = csv.reader(f)
        for row in reader:
